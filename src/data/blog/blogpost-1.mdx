...
<div class="post-wrap stack-lg">
  ...
  <div class="stack-md">
    <ContextBlock elev="none" tone="neutral" text="Support load is spiking around refunds. Finance has events, ops has milestones, but buyers see a black box. A small communication fix beats scaling headcount and prevents churn." />
    <EvidenceList elev="none" tone="blue" items={[
  ...
  <div class="lane lane--lime">
    <SolutionBlock elev="none" tone="lime" text="One Refund view shows stage, a conservative ETA window, and last change. Read-only; no new orchestration. A clear window into systems we already run." />
  </div>

  <ScopeTwoUp elev="none"
    mvp={["Read-only Refund screen per order","API exposing state + ETA window","Event logging for funnel/latency"]}
    nonGoals={["Change refund destination","Live agent chat","Courier/bank deep integrations v1"]}
  />

  <FlowSteps elev="none" steps={[
  ...
  <div class="lane lane--blue">
    <MetricsBlock elev="sm"
      primaryLabel="Primary impact"
      primaryTarget="≥ 20% drop in 'Where is my refund?' tickets"
      guardrail="Post-return NPS must not drop"
      leading={["% of refund views with time-on-page < 10s (clarity)","Late-flag rate trending down"]}
    />
  </div>

  <NextStepsBlock elev="none" steps={[
  ...
  <AssumptionsBlock elev="none" items={[
